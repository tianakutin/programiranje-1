...
let reverse1 list =
  let rec reverse1_aux acc = function
    | [] -> acc
    | x :: xs -> reverse1_aux (x :: acc) xs
  in reverse1_aux [] list
(*----------------------------------------------------------------------------*]
...
let rec unzip = function
 | [] -> ([], [])
 | (x, y) :: xys -> let (lst1, lst2) = unzip xys in (x :: lst1, y :: lst2)
...
let unzip_tlrec list =
  let rec unzip_tlrec_aux acc1 acc2 = function
   ...
   | (x, y) :: xys -> unzip_tlrec_aux (x :: acc1) (y :: acc2) xys
 in
